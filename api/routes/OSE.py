...
def get_ose_history(start_datetime: datetime, end_datetime: datetime,db: Session = Depends(get_db)):
    ...
    observations = (
        db.scalars(
            ...
            .filter(and_(MeterObservations.timestamp >= start_datetime, MeterObservations.timestamp <= end_datetime))
        )
        .unique()
        .all()
    )
    ...
    activity_dates = list(map(lambda activity: activity.timestamp_end.date(), activities_list))
    observation_dates = list(map(lambda observation: observation.timestamp.date(), observations_list))
    ...
    for date_with_history in dates_with_history:
        ...
        activities_on_date = list(filter(lambda activity: activity.timestamp_end.date() == date_with_history, activities_list))
        observations_on_date = list(filter(lambda observation: observation.timestamp.date() == date_with_history, observations_list))

        meters_with_activities_on_date = list(map(lambda activity: activity.meter, activities_on_date))
        meters_with_observations_on_date = list(map(lambda observation: observation.meter, observations_on_date))
        meters_with_history_on_date = set(meters_with_activities_on_date + meters_with_observations_on_date)
        ...
        for meter_with_history in meters_with_history_on_date:
            ...
            meter_activities_on_date = list(filter(lambda activity: activity.meter.id == meter_with_history.id, activities_on_date))
            meter_observations_on_date = list(filter(lambda observation: observation.meter.id == meter_with_history.id, observations_on_date))
            ...
            for meter_activity in meter_activities_on_date:
                ...
                parts_used_strings = list(map(lambda part:  f"{part.part_type.name} ({part.part_number})", meter_activity.parts_used))
                services_performed_strings = list(map(lambda service: service.service_name, meter_activity.services_performed))
            ...
            meter_history = MeterHistoryDTO(
                ...
                location=meter_location.name if meter_location != None else "N/A (NO LOCATION)",
