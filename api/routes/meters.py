...
async def update_meter_type(updated_meter_type: meter_schemas.MeterTypeLU, db: Session = Depends(get_db)):
    _patch(db, MeterTypeLU, updated_meter_type.id, updated_meter_type)

    meter_type = db.scalars(
        select(MeterTypeLU)
        .where(MeterTypeLU.id == updated_meter_type.id)
    ).first()
...
async def create_meter_type(new_meter_type: meter_schemas.MeterTypeLU, db: Session = Depends(get_db)):
