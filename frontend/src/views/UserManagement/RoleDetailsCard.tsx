...
export default function RoleDetailsCard({selectedRole, roleAddMode}: RoleDetailsCardProps ) {
    const { handleSubmit, control, setValue, reset, watch, formState: { errors }} = useForm<UserRole>({
    ...
    return (
        <Card>
            <CardHeader
                title={
                    roleAddMode ?
                        <div className="custom-card-header"><span>Create Role</span><AddIcon style={{fontSize: '1rem'}}/> </div> :
                        <div className="custom-card-header"><span>Edit Role</span><EditIcon style={{fontSize: '1rem'}}/> </div>
                }
                sx={{mb: 0, pb: 0}}
            />
            <CardContent>
                <Grid container>
                        ...
                        <Grid container item>
                            ...
                            <Grid item xs={12} sx={{mt: 2}}>
                                ...
                                <Select
                                    ...
                                    onChange={(event: any) => addSecurityScope(event.target.value[event.target.value.length - 1])}
                                ...
                                {securityScopeList.data?.
                                    filter(x => !watch("security_scopes")?.map(scope => scope.id).includes(x.id))
                                    .map((scope: SecurityScope) =>
                                         <MenuItem value={scope.id}>{scope.scope_string}</MenuItem>
                                )}
                                </Select>
                                </FormControl>
                            </Grid>
                    ...
                    <Grid container item xs={12} sx={{mt: 2}}>
                        {hasErrors() ? <Alert severity="error" sx={{width: '50%'}}>Please correct any errors before submission.</Alert> :
                            roleAddMode ?
                            <Button color="success" variant="contained" onClick={handleSubmit(onAddPart, onErr)}><SaveIcon sx={{fontSize: '1.2rem'}}/>&nbsp; Save New Role</Button> :
