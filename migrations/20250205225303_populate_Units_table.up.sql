WITH
    inserted_rows AS (
        INSERT INTO public."Units"
            (id, name, name_short, description)
        VALUES
            ...
            (
                ...
                'ug/L',
                'Concentration of substance in a water sample.'
            ),
            ...
            (
                ...
                'Barrels per minute',
                'barrels/min',
                'Flow rate in barrels/min'
            )
        ON CONFLICT (id) DO NOTHING
        RETURNING id
    )
SELECT setval('public."Units_id_seq"', COALESCE(MAX(id), 1), TRUE)
FROM inserted_rows;
